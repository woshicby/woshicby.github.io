...
function loadGeoData() {
  ...
  // 并行加载世界和中国省份 GeoJSON
  var worldPromise = fetch('JSON/world.zh.json').then(function(r) { return r.json(); }).catch(function() { return null; });
  var chinaPromise = fetch('JSON/china_provinces.json').then(function(r) { return r.json(); }).catch(function() { return null; });
  ...
  table.innerHTML = `
    ...
    <tbody>
      ${filteredActivities.map(activity => `
        <tr class="run-row">
          <td class="run-date"><a href="activity.html?id=${activity.run_id}">${activity.start_date_local.substring(0, 10)}</a></td>
          <td class="run-time"><a href="activity.html?id=${activity.run_id}">${activity.start_date_local.substring(11, 19)}</a></td>
          <td><a href="activity.html?id=${activity.run_id}">${typeDisplay(activity.type)}</a></td>
          <td><a href="activity.html?id=${activity.run_id}">${formatDistanceWithUnit(activity.distance)}</a></td>
          <td><a href="activity.html?id=${activity.run_id}">${formatPace(activity.distance, activity.moving_time)}/公里</a></td>
          <td><a href="activity.html?id=${activity.run_id}">${formatMovingTime(activity.moving_time)}</a></td>
          <td><a href="activity.html?id=${activity.run_id}">${activity.average_heartrate ? Math.round(activity.average_heartrate) : '-'}</a></td>
        </tr>
      `).join('')}
    </tbody>
  `;
