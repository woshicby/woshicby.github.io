...
from config import (
    IGNORE_BEFORE_SAVING, INDOOR_SPREAD_THRESHOLD, LOOP_CLOSE_THRESHOLD,
    EARTH_RADIUS_M, MAX_ROUTE_POINTS, INDOOR_SUBTYPES, TINY_SPREAD_THRESHOLD,
    MIN_SEGMENT_DISTANCE_M, INDOOR_DISTANCE_THRESHOLD_M, STRAVA_SYNC_LOOKBACK_DAYS,
)
...
def _is_loop(coords):
    ...
    return d < LOOP_CLOSE_THRESHOLD


def _same_city_area(loc1, loc2):
    """Check if two location strings refer to the same city area.

    Compares the last two parts of the comma-separated location string
    (typically city + country) to determine if two activities are in the
    same general area. This prevents indoor activities from getting routes
    from a completely different city.
    """
    if not loc1 or not loc2:
        return True  # If either is unknown, allow replacement
    parts1 = [p.strip() for p in loc1.split(",") if p.strip()]
    parts2 = [p.strip() for p in loc2.split(",") if p.strip()]
    # Compare the last two components (city-level granularity)
    tail1 = parts1[-2:] if len(parts1) >= 2 else parts1
    tail2 = parts2[-2:] if len(parts2) >= 2 else parts2
    return tail1 == tail2
...
def _build_route_for_distance(ref_coords, target_m):
    ...
    for pt in it:
        ...
        if len(result) > MAX_ROUTE_POINTS:
            break
...
class Generator:
    ...
    def sync(self, force):
        ...
        if force:
            ...
            if last_activity:
                ...
                last_activity_date = last_activity_date.shift(days=-STRAVA_SYNC_LOOKBACK_DAYS)
    ...
    def sync_from_data_dir(self, data_dir, file_suffix="gpx", activity_title_dict={}):
        ...
        print(f"成功加载 {len(tracks)} 条有效轨迹")
        if not tracks:
            print("[警告] 没有找到有效的轨迹数据")
            return
        ...
        for t in tracks:
            try:
                ...
                error_count += 1
                file_name = t.file_names[0] if t.file_names else "unknown"
                print(f"\n[错误] 写入数据库失败: {file_name}")
        ...
        print(f"\n数据库写入完成: 成功 {success_count} 条, 失败 {error_count} 条")

    def load(self):
        # if sub_type is not in the db, just add an empty string to it
        query = self.session.query(Activity).filter(Activity.distance >= 0)
        if self.only_run:
    ...
    def _fix_indoor_locations(activity_list):
        ...
        for a in activity_list:
            ...
            if not is_indoor and coords is None and a.get("distance", 0) > INDOOR_DISTANCE_THRESHOLD_M:
                is_indoor = True
        ...
        # Only replace if the outdoor route is in a similar location area
        ...
        for a, is_indoor, coords in classified:
            if not is_indoor:
                ...
                # Only replace polyline if the outdoor activity is in the
                # same city area. Compare location_country at city level.
                act_loc = a.get("location_country", "")
                same_area = (
                    not act_loc
                    or not last_outdoor_location
                    or _same_city_area(act_loc, last_outdoor_location)
                )
                if last_outdoor_coords is not None and same_area:
                    ...
                    # No suitable outdoor route found; mark as indoor
                    # but keep original (empty) polyline
                    a["subtype"] = "indoor"
