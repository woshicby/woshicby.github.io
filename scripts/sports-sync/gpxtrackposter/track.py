...
class Track:
    ...
    def load_fit(self, file_name):
        try:
            ...
            if errors:
                print(
                    f"[错误] FIT 文件读取失败: {os.path.basename(file_name)}"
                )
                print(f"  错误详情: {errors}")
                print(f"  该文件已损坏，将被跳过")
                return
            if messages.get("session_mesgs") is None:
                print(
                    f"[警告] FIT 文件缺少 session 数据: {os.path.basename(file_name)}"
                )
                print(f"  该文件将被跳过")
                return
            self._load_fit_data(messages)
        except TrackLoadError as e:
            print(f"[错误] FIT 文件加载失败: {os.path.basename(file_name)}")
            print(f"  错误详情: {e}")
        except Exception as e:
            print(f"[错误] FIT 文件加载异常: {os.path.basename(file_name)}")
    ...
    def _load_fit_data(self, fit: dict):
        ...
        sport = message.get("sport", "generic").lower()
        if sport == "running":
            self.type = "Run"
        else:
            self.type = sport
        ...
        self.moving_dict["moving_time"] = datetime.timedelta(
            seconds=(
                ...
                or message.get("total_timer_time", 0)
            )
        )
    ...
    def append(self, other):
        ...
        try:
            ...
            self.moving_dict["average_speed"] = (
                self.moving_dict["distance"] / moving_seconds
                if moving_seconds > 0
                else 0
            )
