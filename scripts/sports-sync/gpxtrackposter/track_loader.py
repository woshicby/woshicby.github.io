...
from synced_data_file_logger import get_synced_data_file_list
...
class TrackLoader:
    ...
    def load_tracks(self, data_dir, file_suffix="gpx", activity_title_dict={}):
        ...
        # filter out tracks with length < min_length (but allow 0-distance activities)
        return [t for t in tracks if t.length >= self.min_length or t.length == 0]
