           ...
           if (race.category === '越野跑') {
               ...
               prItem.innerHTML = `
                   ...
                   <div class="pr-details">
                       <div class="pr-result">
                           <strong>最佳赛事分:</strong> <span class="race-time">${race.raceScore}</span>
                       </div>
                       <div class="pr-pace">
...
document.addEventListener('DOMContentLoaded', async () => {
   try {
       ...
       console.log('已从JSON文件加载赛事记录数据:', raceRecords.length, '条记录');
       generateRaceRecords();
       // 加载并处理完赛证书数据
...
function generateCertificates(certificates) {
   ...
   const items = certificates.map(certificate => {
       ...
       certificateDate.textContent = `赛事日期: ${certificate.date}`;
