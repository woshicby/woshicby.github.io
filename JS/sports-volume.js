...
function renderVolumePage() {
  ...
  renderYearSummarySidebar(sportType, interval);
...
function renderActivityCards(container, sportType) {
  ...
  keys.forEach(function(key) {
    ...
    acts.forEach(function(a) {
      ...
      html += '<a href="sports-activity.html?id=' + a.run_id + '" class="vc-row-link">';
...
function renderYearSummarySidebar(sportType, interval) {
  ...
  var filteredActivities = filterBySportType(activities, sportType || 'all');
  ...
  switch(interval || 'year') {
    ...
    case 'day':
      ...
      filteredActivities.forEach(function(a) {
        ...
        if (dateStr.length >= 10) {
          var key = dateStr.substring(0, 10);
          if (!groups[key]) groups[key] = [];
          groups[key].push(a);
        }
      });
    ...
    default:
      ...
      maxItems = 10;
      filteredActivities.forEach(function(a) {
        var year = extractYear(a.start_date_local);
        if (year) {
  ...
  var groupList = Object.keys(groups).sort().reverse();
  if (!groupList.length) {
    container.innerHTML = '<div class="summary-loading">暂无运动数据</div>';
    return;
  }

  var allActivities = [];
  groupList.forEach(function(g) { allActivities = allActivities.concat(groups[g]); });
  
  var totalRuns = allActivities.length;
  var totalDistM = allActivities.reduce(function(s,a){ return s + (a.distance||0); }, 0);
  var totalDistKm = Math.round(totalDistM / 1000);
  var totalTimeS = allActivities.reduce(function(s,a){ return s + convertTimeToSeconds(a.moving_time); }, 0);
  var totalHours = Math.floor(totalTimeS / 3600);
  var longestKm = Math.max.apply(null, allActivities.map(function(a){ return (a.distance||0)/1000; }));
  var avgPace = "0'00\"";
  if (totalDistM > 0 && totalTimeS > 0) {
    var paceS = totalTimeS / totalDistKm;
    avgPace = Math.floor(paceS/60) + "'" + ("0" + Math.floor(paceS%60)).slice(-2) + '"';
  ...
  var html = '<div class="ys-header">总计</div>';
  html += '<div class="ys-stats-grid">';
  html += ysStatItem('距离', totalDistKm + '', '公里');
  html += ysStatItem('次数', totalRuns + '', '次');
  html += ysStatItem('时长', totalHours + '', '小时');
  html += ysStatItem('最长', longestKm.toFixed(1), '公里');
  html += ysStatItem('配速', avgPace, '');
  html += '</div>';

  groupList.forEach(function(groupKey) {
    var groupTracks = groups[groupKey];
    var groupDistM = groupTracks.reduce(function(s,a){ return s + (a.distance||0); }, 0);
    var groupDistKm = Math.round(groupDistM / 1000);
    var groupRuns = groupTracks.length;
    
    var displayLabel = '';
    if (interval === 'month') {
      displayLabel = groupKey.substring(0, 4) + '-' + groupKey.substring(5) + '月';
    } else if (interval === 'week') {
      displayLabel = groupKey;
    } else if (interval === 'day') {
      displayLabel = groupKey.substring(5);
    } else {
      displayLabel = groupKey + '年';
    }
    
    html += '<div class="year-summary-item">';
    html += '<div class="ys-header">' + displayLabel + '</div>';
    html += '<div class="ys-stats-grid">';
    html += ysStatItem('距离', groupDistKm + '', '公里');
    html += ysStatItem('次数', groupRuns + '', '次');
    html += '</div>';
    html += '</div>';
  });
